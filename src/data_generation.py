...
- Relaciones:
  * Precio base por m² según zona (zonas premium más caras).
  ...
  * Distribuciones diferentes por zona (p.ej., más apartamentos en Zona 10/14; más casas en Zona 16/Carretera al Salvador).
Uso:
...
class ZoneConfig:
    ...
    weight: float              # Probabilidad de muestreo de la zona
    apt_share: float           # Proporción de apartamentos vs casas
...
# Configuración de zonas (ficticia, pero con sentido: Premium > Intermedias > Periféricas)
ZONES: Dict[str, ZoneConfig] = {
    # Premium / corporativas
    "Zona 10": ZoneConfig(base_price_m2=145, weight=0.16, apt_share=0.85, center_to_business_km=1.0, typical_security=0.9, typical_amenities=0.8),
    "Zona 14": ZoneConfig(base_price_m2=155, weight=0.12, apt_share=0.9,  center_to_business_km=1.2, typical_security=0.95, typical_amenities=0.85),
    "Zona 15": ZoneConfig(base_price_m2=135, weight=0.10, apt_share=0.7,  center_to_business_km=2.5, typical_security=0.85, typical_amenities=0.7),
    "Zona 16": ZoneConfig(base_price_m2=125, weight=0.10, apt_share=0.55, center_to_business_km=4.0, typical_security=0.8,  typical_amenities=0.6),

    # Corredor hacia las afueras (casas más grandes, vida residencial)
    "Carretera al Salvador": ZoneConfig(base_price_m2=110, weight=0.13, apt_share=0.35, center_to_business_km=8.0, typical_security=0.75, typical_amenities=0.5),

    # Intermedias / mixtas
    "Zona 4":  ZoneConfig(base_price_m2=120, weight=0.08, apt_share=0.8,  center_to_business_km=2.0, typical_security=0.8,  typical_amenities=0.7),
    "Zona 9":  ZoneConfig(base_price_m2=125, weight=0.07, apt_share=0.85, center_to_business_km=1.5, typical_security=0.85, typical_amenities=0.75),
    "Zona 13": ZoneConfig(base_price_m2=120, weight=0.06, apt_share=0.8,  center_to_business_km=2.5, typical_security=0.8,  typical_amenities=0.65),

    # Más asequibles / periféricas urbanas
    "Zona 5":  ZoneConfig(base_price_m2=95,  weight=0.06, apt_share=0.6,  center_to_business_km=3.5, typical_security=0.65, typical_amenities=0.4),
    "Zona 7":  ZoneConfig(base_price_m2=85,  weight=0.06, apt_share=0.5,  center_to_business_km=4.5, typical_security=0.55, typical_amenities=0.35),
    "Zona 18": ZoneConfig(base_price_m2=75,  weight=0.06, apt_share=0.45, center_to_business_km=6.0, typical_security=0.5,  typical_amenities=0.3),
}
...
def _size_and_rooms(prop_type: str, zone: str, rng: np.random.Generator) -> Tuple[int, int, int]:
    """Tamaños coherentes por tipo/zonas (apartamentos más compactos en zonas premium)."""
    if prop_type == "Apartamento":
        # Apartamentos más compactos en zonas premium, algo mayores en zonas periféricas
        base_mu = 72 if zone in ["Zona 10", "Zona 14", "Zona 9"] else 85
        base_sigma = 18
        size = int(np.clip(rng.normal(base_mu, base_sigma), 35, 180))
    else:
        # Casas más grandes, especialmente en Carretera al Salvador / Zona 16 / 15
        base_mu = 150 if zone in ["Carretera al Salvador", "Zona 16", "Zona 15"] else 130
        size = int(np.clip(rng.normal(base_mu, 40), 70, 350))

    # Dormitorios y baños proporcionales al tamaño
    bedrooms = int(np.clip(round(size / (prop_type == "Apartamento" and 28 or 35)), 1, 6))
...
def _floors_and_age(prop_type: str, zone: str, rng: np.random.Generator) -> Tuple[int, int, int]:
    ...
    if prop_type == "Apartamento":
        # Edificios más altos en 10/14/9
        max_floor = 18 if zone in ["Zona 10", "Zona 14", "Zona 9"] else 10
        floor = int(rng.integers(1, max_floor + 1))
        has_elevator = 1 if (floor >= 4 or zone in ["Zona 10", "Zona 14", "Zona 9", "Zona 4"]) else int(rng.random() < 0.5)
    else:
        floor = 0
        has_elevator = 0

    # Antigüedad: zonas premium tienden a ser más nuevas (desarrollos recientes)
    base_age = 12 if zone in ["Zona 10", "Zona 14", "Zona 15", "Zona 16"] else 18
...
def _amenities(zone: str, rng: np.random.Generator) -> Dict[str, int]:
    ...
    furnished = int(rng.random() < 0.25 if zone in ["Zona 10", "Zona 14"] else rng.random() < 0.12)
    balcony = int(rng.random() < 0.55)
    garden = int(rng.random() < (0.15 if zone in ["Zona 10", "Zona 14", "Zona 9"] else 0.35))  # más común en casas/periferia
...
def _environment(zone: str, rng: np.random.Generator) -> Tuple[float, float, int]:
    ...
    # Vista: mejor en pisos altos o casas con áreas verdes (Zona 15/16/Carretera)
    if zone in ["Zona 15", "Zona 16", "Carretera al Salvador"]:
        base_view = 3.2
    else:
        base_view = 2.6
    view_quality = int(np.clip(round(base_view + rng.normal(0.0, 0.9)), 1, 5))

    # Ruido: mayor en zonas céntricas y vías principales
    base_noise = 3.4 if zone in ["Zona 10", "Zona 9", "Zona 4", "Zona 13"] else 2.6
    noise_level = int(np.clip(round(base_noise + rng.normal(0.0, 0.9)), 1, 5))
    return center_km, transit_km, view_quality - noise_level  # también devolvemos un score sencillo de “atractivo exterior”
...
def _price_formula(row: pd.Series, rng: np.random.Generator) -> float:
    ...
    # Multiplicadores
    m_type = 1.00 if row["property_type"] == "Apartamento" else 0.95  # casas algo menos por m², pero suelen ser más grandes
    m_age = 1.0 - min(row["age_years"], 40) * 0.006  # deprecia ~0.6% por año hasta 40
    m_view = 1.0 + (row["view_quality"] - 3) * 0.03
    m_noise = 1.0 - (row["noise_level"] - 3) * 0.025

    # Premium por piso alto en zonas verticales (si tiene elevador)
    ...
    # Ajuste por dormitorios/baños si hay desbalance (más cuartos en poco metraje → resta, y viceversa)
...
def generate_rent_data(n: int = 3000, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(1, n + 1):
        ...
        noise_level = np.clip(3 + (-ext_score) + rng.normal(0, 0.6), 1, 5)  # derivar ruido de ext_score (inverso)
    ...
    df["is_premium_zone"] = df["zone"].isin(["Zona 10", "Zona 14", "Zona 15"]).astype(int)
...
def main():
    ...
    parser.add_argument("--n", type=int, default=3000, help="Número de registros a generar")
    ...
    print("\nTop 5 zonas por precio promedio (GTQ):")
